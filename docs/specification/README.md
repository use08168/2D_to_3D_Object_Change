...
| `13_workspace_integration.ipynb` | [13-15_workspace.md](13-15_workspace.md) | 분산 앵커 작업공간: 마커 지도·로컬라이제이션·검증 |
| `14_realtime_workspace.ipynb` | [13-15_workspace.md](13-15_workspace.md) | 웹캠 실시간 작업공간 루프 |
| `15_realtime_tophat.ipynb` | [13-15_workspace.md](13-15_workspace.md) | **top-hat 국소대비 검출 + 방향 정렬 + 인터랙티브 3D(현재 방향)** |
| `mobile/` + `src/stream_server.py` | [16_phone_stream.md](16_phone_stream.md) | **폰 카메라 → PC 실시간 스트리밍(Expo Go + Tailscale)** |

보조: `alt_generate_markers.ipynb`(마커 생성), `alt_markers_to_3d.ipynb`(마커 3D 배치 초기 실험).

> **현재 방향(13~15)**: 소형 보드 → **넓은 작업공간(분산 마커 지도)**. 보이는 마커 4~5개만으로 로컬라이즈, top-hat 국소대비로 어두운/낮은 물체까지 검출, 실제 방향 정렬 + 마우스 인터랙티브 3D. 상세: [13-15_workspace.md](13-15_workspace.md).
...
7. **소형 보드의 공간 제약** → **넓은 작업공간(분산 마커 지도) 로컬라이제이션**(13, 최소 4~5개).
8. **절대높이 검출의 약점**(낮은/어두운 물체 놓침, 드리프트 가짜블롭) → **top-hat 국소대비 + 봉우리 검증**(15).
9. **가상 3D 방향 불일치** → 실제 카메라 포즈 정렬(OpenCV `_lookat_aligned`) + plotly chirality 교정(15).
10. **웹캠의 위치 제약** → **폰 카메라 스트리밍**(16, Expo Go + WebSocket + Tailscale 고정주소·P2P).
